...
const HotelReg = () => {
  ...
  return (
    <div onClick={() => setShowHotelReg(false)} className='fixed top-0 bottom-0 left-0 right-0 z-50 flex items-center justify-center bg-black/70'>
      <form onSubmit={onSubmitHandler} onClick={(e) => e.stopPropagation()} className='flex bg-white rounded-xl max-w-4xl max-md:mx-2 w-full'>
        <img
          src={assets.regImage}
          alt='reg-image'
        ...
        <div className='relative flex flex-col items-center md:w-1/2 p-8 md:p-10'>
          ...
          <button
            type="submit"
            disabled={!name || !address || !contact || !city}
            className={`bg-indigo-500 hover:bg-indigo-600 transition-all text-white mr-auto px-6 py-2 rounded cursor-pointer mt-6 ${(!name || !address || !contact || !city) ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            Register
          </button>
        </div>
