...
const RadioButton = ({label, selected = false , onChange =() =>{}})=>{
 return (
    <label className='flex gap-3 items-center cursor-pointer mt-2 text-sm'> <input type="radio" name ="sortOption" checked={selected} onChange={()=> onChange(e.target.checked, label)}/>
    <span className='font-light select-none'>{label}</span>
    </label>
 )
}
...
const AllRooms = () => {
   ...
   const roomTypes =[ "Single bed", "Double bed", "Suite", "Deluxe", "Family Room"];
  ...
  <div>
    <div className='flex flex-col items-start text-left'>
        <h1 className='font-playfair text-4xl md:text-5xl md:text-[40px] '>Hotel Rooms</h1>
...
<p className ='ml-2'> 200+reviews</p>
        ...
        <div className ='flex items-center gap-11 text-gray-500 mt-2 text-sm'>
